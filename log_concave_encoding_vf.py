...
def is_distribution(distribution): #is the given array a distribution ? Must be normalized (norm 1)
...
def angles(distribution, n_qubits):       
    ...
    if is_log_concave_encoding_compatible(distribution, n_qubits) == True:
        ...
        for step in range(n_qubits):
            inter = 2**(step)
            limit_region = (size / (inter * 2)) * np.arange(inter * 2 + 1)
            inter_list = []
            for region in range(inter):
                if  euclidean_norm(distribution[int(limit_region[2 * region]):int(limit_region[2 * region + 1])]) == 0:
                    inter_list.append(np.pi/2)
                else:
                    inter_list.append(np.arctan2(euclidean_norm(distribution[int(limit_region[2 * region + 1]):int(limit_region[2 * region + 2])]), euclidean_norm(distribution[int(limit_region[2 * region]):int(limit_region[2 * region + 1])])))
...
def log_concave_encoding(distribution, n_qubits):
    ...
    for step in range(n_qubits - 1):
        ...
        for region in range(2 ** step):
            ...
            circuit.mcry(2 * theta[step][region], controller_qubits, qubits[n_qubits - step - 1], None)
            circuit = x_gates_region(circuit, to_bin(region, step))        
    return circuit
